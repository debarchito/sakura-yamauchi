...
const command: Command.Init = {
  ...
  async execute({ args, msg, share: { choose } }) {
    ...
    let message: string;
    if (!len) message = "Didn't receive any arguments while expecting at least 2 arguments!";
    else if (len === 1) message = "Received one argument while expecting at least 2 arguments!";
